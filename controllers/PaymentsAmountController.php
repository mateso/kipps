...
class PaymentsAmountController extends Controller
{
    ...
    protected function findModel($id, $payments_id, $payment_setup_id)
    {
     ...
     public function actionFeeStatement() {
        $searchModel = new PaymentsAmountSearch;
        $condition = "payment_setup_id IN (SELECT id FROM payment_setup WHERE payment_type=1 AND financial_year_id=".Yii::$app->session->get('mfy')->YearID.") AND payments_id NOT IN (SELECT `id` FROM `payments` WHERE `status` = 0)";
    ...
    public function actionTransportPayments() {
        $searchModel = new PaymentsAmountSearch;
        $condition = "payment_setup_id IN (SELECT id FROM payment_setup WHERE payment_type=2 AND financial_year_id=".Yii::$app->session->get('mfy')->YearID.") AND payments_id NOT IN (SELECT `id` FROM `payments` WHERE `status` = 0)";
        $dataProvider = $searchModel->search(Yii::$app->request->getQueryParams(), $condition);
    ...
    public function actionUniformPayments() {
        $searchModel = new PaymentsAmountSearch;
        $condition = "payment_setup_id IN (SELECT id FROM payment_setup WHERE payment_type=3 AND financial_year_id=".Yii::$app->session->get('mfy')->YearID.") AND payments_id NOT IN (SELECT `id` FROM `payments` WHERE `status` = 0)";
        $dataProvider = $searchModel->search(Yii::$app->request->getQueryParams(), $condition);
    ...
    public function actionApplicationPayments() {
        $searchModel = new PaymentsAmountSearch;
        $condition = "payment_setup_id IN (SELECT id FROM payment_setup WHERE payment_type=4 AND financial_year_id=".Yii::$app->session->get('mfy')->YearID.") AND payments_id NOT IN (SELECT `id` FROM `payments` WHERE `status` = 0)";
        $dataProvider = $searchModel->search(Yii::$app->request->getQueryParams(), $condition);
    ...
    public function actionAdmissionPayments() {
        $searchModel = new PaymentsAmountSearch;
        $condition = "payment_setup_id IN (SELECT id FROM payment_setup WHERE payment_type=9 AND financial_year_id=".Yii::$app->session->get('mfy')->YearID.") AND payments_id NOT IN (SELECT `id` FROM `payments` WHERE `status` = 0)";
        $dataProvider = $searchModel->search(Yii::$app->request->getQueryParams(), $condition);
    ...
    public function actionOtherPayments() {
        $searchModel = new PaymentsAmountSearch;
        $condition = "payment_setup_id IN (SELECT id FROM payment_setup WHERE payment_type IN (5,6,7,8) AND financial_year_id=".Yii::$app->session->get('mfy')->YearID.") AND payments_id NOT IN (SELECT `id` FROM `payments` WHERE `status` = 0)";
        $dataProvider = $searchModel->search(Yii::$app->request->getQueryParams(), $condition);
