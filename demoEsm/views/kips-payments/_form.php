    ...
    echo Form::widget([
        ...
        'attributes' => [
            'student_id' => ['type' => Form::INPUT_WIDGET, 'widgetClass' => \kartik\select2\Select2::className(), 'options' => [
                    'data' => \yii\helpers\ArrayHelper::map(\app\models\KipsUsers::findBySql("SELECT `userid`,CONCAT(`firstname`,' ',`middlename`,' ',`surname`) AS fullname FROM `user` WHERE user_type =2")->orderBy('userid')->asArray()->all(), 'userid', 'fullname'), 'options' => ['prompt' => 'Select Student'],
                    'pluginOptions' => [
                        'allowClear' => true,
                    ]
                ]],
