...
class KipsPaymentsController extends Controller {
    ...
    public function actionCreate() {
        ...
        if (Yii::$app->request->post()) {
            ...
            $model->validate();
            echo '<pre>';print_r($model->errors);die('Hapaaaaa');
            if ($model->save()) {
                //tution_fee
                $tution_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` = 1 AND `education_level` = " . $student->class . " AND `fee_category` = " . $model->student_type)->one();
                if ($tution_fee_setup) {
                ...
                if ($model->transport_route) {
                    $transport_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` = 2 AND transport_routes = " . $model->transport_route)->one();
                ...
                //graduation_fee
                $graduation_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` =7")->one();
                ...
                //uniform_fee
                if ($student->class <= 2) {
                    $uniform_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` =3  AND `education_level` = " . $student->class)->one();
                } else {
                    $uniform_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` =3  AND `education_level` = " . $student->class . " AND `fee_category` = " . $model->student_type)->one();
                }
                ...
                //admission_fee
                if ($student->class <= 2) {
                    $admission_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` =9  AND `education_level` = " . $student->class)->one();
                } else {
                    $admission_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` =9  AND `education_level` = " . $student->class . " AND `fee_category` = " . $model->student_type)->one();
                }
                ...
                if ($student->class = 5 || $student->class = 8) {
                    $examination_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` =6  AND `education_level` = " . $student->class)->one();
                ...
                //remedial_fee
                if ($student->class = 8) {
                    $remedial_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` =6  AND `education_level` = " . $student->class)->one();
                ...
                //study_tour_fee
                 $study_tour_fee_setup = \app\models\KipsPaymentSetup::findBySql("SELECT * FROM `payment_setup` WHERE `financial_year_id` = 5 AND `payment_type` =5 ")->one();
